...
def index():
    ...
    logger.info(f"Serving iam_viewer_connected.html with timestamp {timestamp}")
    
    response = make_response(render_template('iam_viewer_connected.html', cache_bust=timestamp))
...
def smiles_to_xyz():
    ...
    try:
        ...
        try:
            ...
            return jsonify({'success': False, 'error': f'SMILES conversion failed: {str(e)}'})
